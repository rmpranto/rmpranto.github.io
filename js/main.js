...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Always run the timeline animation regardless of config availability
    initTimelineScroll();
});
...
/**
 * Timeline scroll animation — Smooth continuous progress line approach.
 */
function initTimelineScroll() {
    const wrappers = document.querySelectorAll('.timeline-wrapper');
    if (!wrappers.length) return;

    // Remove any leftover .tl-fill divs from previous approach
    document.querySelectorAll('.tl-fill').forEach(el => el.remove());

    wrappers.forEach(wrapper => {
        if (!wrapper.querySelector('.timeline-progress-line')) {
            const line = document.createElement('div');
            line.className = 'timeline-progress-line';
            // Prepend so it sits below items in the DOM stacking order
            wrapper.insertBefore(line, wrapper.firstChild);
        }
    });

    const updateTimelines = () => {
        // The "drawing tip" height where the line hits as you scroll (85% down viewport)
        const drawPoint = window.scrollY + (window.innerHeight * 0.85);
        // Add a check for hitting the bottom of the page
        ...
        wrappers.forEach(wrapper => {
            ...
            // Track starts 22px from top, ends 22px from bottom (matching CSS ::before)
            const trackStartDoc = wrapperTopDoc + 22;
            const maxTrackHeight = wrapper.offsetHeight - 44; // 22px top + 22px bottom margin

            let currentProgress = drawPoint - trackStartDoc;

            // If the user has scrolled to the very bottom of the page, fill any visible timeline to the end.
            const wrapperBottomDoc = wrapperTopDoc + wrapper.offsetHeight;
            if (isAtBottom && (window.scrollY + window.innerHeight) > wrapperBottomDoc - 100) {
                currentProgress = maxTrackHeight;
            }

            if (currentProgress < 0) currentProgress = 0;
            if (currentProgress > maxTrackHeight) currentProgress = maxTrackHeight;

            line.style.height = `${currentProgress}px`;

            const items = wrapper.querySelectorAll('.timeline-item');
            items.forEach((item, index) => {
                const icon = item.querySelector('.timeline-icon');
                const itemTopDoc = item.getBoundingClientRect().top + window.scrollY;
                const iconCenterDoc = itemTopDoc + 22;

                // Determine where this item's territory ends
                const nextItem = items[index + 1];
                const nextIconCenterDoc = nextItem
                    ? (nextItem.getBoundingClientRect().top + window.scrollY + 22)
                    : (wrapperBottomDoc);

                if (drawPoint >= iconCenterDoc || isAtBottom) {
                    item.classList.add('tl-active');
                    if (icon) {
                        icon.classList.add('active');
                        // Only blink if it's the CURRENT item being passed (or the very last one at the bottom)
                        if (drawPoint < nextIconCenterDoc || (isAtBottom && !nextItem)) {
    ...
    // slight delay to let DOM settle layout calculations
    setTimeout(updateTimelines, 100);
}
